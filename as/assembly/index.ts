...
export function init(ignored: i32, offset: i32, w: i32, h: i32): void {
  ...
  height = h;
  area = width * height;
  status = new Pointer(HEAP_BASE + offset + 8 * area);
  image  = new Pointer(HEAP_BASE + offset);
  force  = new Pointer(HEAP_BASE + offset + 4  * area);
  status = new Pointer(HEAP_BASE + offset + 8  * area);
  u      = new Pointer(HEAP_BASE + offset + 12 * area);
  v    = new Pointer(HEAP_BASE + offset + 16 * area);

// emcc emscripten.c -Os -s WASM=1 -s SIDE_MODULE=1 -o emscripten.wasm
...
EMSCRIPTEN_KEEPALIVE int* getByteOffset() {
  return &heap[0];
}
...
EMSCRIPTEN_KEEPALIVE void init(int *arr, int offset, int w, int h) {
  width = w;
  heap = arr;
  heapStart = offset;
...
EMSCRIPTEN_KEEPALIVE void step(signalAmplitude, dampingBitShift) {
